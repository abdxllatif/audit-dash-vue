  ...
  methods: {
    ...
    async getData () {
      if (this.id) {
        ...
        await axios.get('http://localhost:8090/api/data/formations/' + this.id, { headers: { 'x-access-token': this.$session.get('jwt') } })
          .then(r => {
            console.log('Last:')
    ...
    submit () {
      ...
      setTimeout(() => {
        ...
        dayjs.extend(utc)
        axios.post('http://localhost:8090/api/data/formations/' + this.id, {
          nom: this.form.nom,
