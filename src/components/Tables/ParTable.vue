...
export default {
  ...
  methods: {
    ...
    trashConfirm () {
      ...
      axios.delete('http://localhost:8090/api/data/partenaires/' + this.trashObject.partenaireId + '/' + this.$store.state.userEmail, { headers: { 'x-access-token': this.$session.get('jwt') } })
        .then(r => {
          this.isLoading = false
          this.$buefy.toast.open({
            message: 'Confirmed',
