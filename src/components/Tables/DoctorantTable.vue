    ...
    <b-table
      ...
      <b-table-column custom-key="actions" cell-class="is-actions-cell" v-slot="props">
        <div class="buttons is-right">
          <router-link :to="{name:'doctorantEdit', params: {id: props.row.doctorantId}}" class="button is-small is-primary">
...
export default {
  ...
  methods: {
    ...
    trashConfirm () {
      ...
      axios.delete('http://localhost:8090/api/data/doctorants/' + this.trashObject.activiteId + '/' + this.$store.state.userEmail, { headers: { 'x-access-token': this.$session.get('jwt') } })
        .then(r => {
          this.isLoading = false
          this.$buefy.toast.open({
            message: 'Confirmed',
